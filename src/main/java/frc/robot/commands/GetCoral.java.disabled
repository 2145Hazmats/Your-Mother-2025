// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.commands;

import edu.wpi.first.wpilibj2.command.Command;
import frc.robot.Constants.shooterBoxxContants;
import frc.robot.ReefConstants.PoseConstants;
import frc.robot.subsystems.CommandSwerveDrivetrain;
import frc.robot.subsystems.ElevatorSubsystem;
import frc.robot.subsystems.ShooterBoxx;

/* You should consider using the more terse Command factories API instead https://docs.wpilib.org/en/stable/docs/software/commandbased/organizing-command-based.html#defining-commands */
public class GetCoral extends Command {

  // declare things from other subsystems
  private CommandSwerveDrivetrain theLegs;
  private ShooterBoxx theSnout;

  /** Creates a new GetCoral. */
  // Constructor
  public GetCoral(CommandSwerveDrivetrain theFakeLegs, ShooterBoxx theFakeSnout) {
    theLegs = theFakeLegs;
    theSnout = theFakeSnout;
    
    // Use addRequirements() here to declare subsystem dependencies.
    addRequirements(theFakeSnout);
  
  }

  // Called when the command is initially scheduled.
  @Override
  public void initialize() {
  }

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {
    theSnout.SuckTillSensor();
  }

  // Called once the command ends or is interrupted.
  @Override
  public void end(boolean interrupted) {
    theSnout.StopShooterMotor();
  }

  // Returns true when the command should end.
  @Override
  public boolean isFinished() {
    return theSnout.StopCoralIntake();
  }
}
